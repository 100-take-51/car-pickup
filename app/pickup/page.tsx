    ...
    <>
      <main className="wrap">
        ...
        <section className="card">
          ...
          <h3>エンジンがかからない車</h3>
          <p className="lead">
            自走できない車でもご相談いただけます。状況に応じて引取方法をご案内します。
          </p>

          <h3>長年放置している車</h3>
          <p className="lead">
            長期間動かしていない車についても、状況を確認のうえ対応しています。
          </p>

          <h3>名義が本人ではない車</h3>
          <p className="lead">
            名義が違う場合や、代理の方からのご相談も受け付けています。まずは状況を教えてください。
          </p>

          <h3>相続した車</h3>
          <p className="lead">
            相続された車についてもご相談いただけます。手続きの状況が分からない場合でも構いません。
          </p>
        </section>

        {/* 事故・二次搬送（今回の強み） */}
        <section className="card">
          <h2>事故後に移動できなくなった車の引取について</h2>
          <p className="lead">
            事故を起こした、または事故に遭ったことで移動できなくなった車についてのご相談も多くいただいています。
            事故現場から警察署や指定場所までは保険で移動できても、その後の移動は「二次搬送」となり、
            別途費用が必要になるケースがあります。
          </p>
          <p className="lead">
        ...
        <section className="card">
          ...
          <div style={{ display: "grid", gap: 10 }}>
            {faqItems.map((x) => (
              <details key={x.q} style={{ border: "1px solid rgba(255,255,255,.12)", borderRadius: 12, padding: 10 }}>
                <summary style={{ cursor: "pointer", fontWeight: 700 }}>{x.q}</summary>
                <p className="lead" style={{ marginTop: 8 }}>{x.a}</p>
              </details>
            ))}
          </div>
        </section>
